...
class OpenHand():
    ...
    def __init__(self, port, servo_ids, series="RX"):
        ...
        for servo in self.servos:
            ...
            time.sleep(
                self.pause
            )  #in case eeprom delay is what is causing the issues
            if series == "RX":
                servo.apply_speed(1)
            time.sleep(self.pause)
        ...
        self.modes = [
            True
        ] * num_servos  #default assignment (shouldn't have servos in torque mode during normal operation)

        if len(self.motorDir) != num_servos or len(
            self.motorMin
        ) != num_servos or len(self.motorMax) != num_servos:
        ...
        if num_servos == 4:  #This is the model_O and we want to prevent gear shear
            for i in range(num_servos):
                ...
                if series == "RX":
                    if (self.motorDir[i] == 1 and
                        enc > 512) or (self.motorDir[i] == -1 and enc < 512):
                        print("------------FAILSAFE-------------")
                        print(
                            "Failsafe is incorporated to prevent gear shear in Model O"
                        )
                        print("Motor encoder postion: ", enc)
                        input(
                            "Your encoder position for motor index " + str(i) +
                            " may cause the motor to move backwards and break gears. We recommend you resetting the fingers to prevent gear shear, proceed? [ENTER]"
                        )
                    else:
                        print("Motor directions not set...")
                #These would then be the MX and XM motors
                elif (self.motorDir[i] == 1 and
                      enc > 2048) or (self.motorDir[i] == -1 and enc < 2048):
                    print("------------FAILSAFE-------------")
                    print(
                        "Failsafe is incorporated to prevent gear shear in Model O"
                    )
                    print("Motor encoder postion: ", enc)
                    input(
                        "As an XM Motor, we can automatically fix this issue for motor "
                        + str(i) + ", proceed? [ENTER]"
                    )
                    self.servos[i].enable_extended_position_control_mode()
                    self.servos[i].move_to_encoder(
                        self.servos[i].settings['max_encoder'] + 100
                    )
                    time.sleep(self.pause)
            ...
            if self.abduction_limit != 1:
                self.servos[0].enable_current_position_control_mode(
                    self.abduction_limit
                )
    ...
    def reset(
        self
    ):  #returns everything to zeroed positions, different from release
        print("[ERR] reset() not implemented")
        return False

    def release(
        self
    ):  #opens the finger components, doesn't necessarily move non-finger servos
    ...
    def moveMotor(self, index, amnt):
        if amnt < 0. or amnt > 1.0:
            print(
                "[WARNING] motion out of bounds, capping to [0,1]. Index: "
                + repr(index) + ", Cmd:" + repr(amnt)
            )
        ...
        if (index < 0 or index >= len(self.servos)):
            ...
            if self.motorDir[index] > 0:  #normal case
                servo.move_to_encoder(
                    int(
                        servo.settings["max_encoder"] * (
                            self.motorMin[index] + amnt *
            ...
            if not self.modes[
                index
            ]:  #restore position-control mode if necessary - want to register new encoder target first before re-applying system torque
    ...
    def torqueMotor(self, index, val, pos_val=None):
        val = min(
            1.0, max(val, 0)
        )  #by design, can exceed default max torque value
        ...
        if pos_val is None:
            ...
            pos_val = min(
                max(0, pos_val), 1.0
            )  #new target position is always max to force saturation of servo torque
            enc = int(
                (
                    pos_val * (self.motorMax[index] - self.motorMin[index])
                    + self.motorMin[index]
                ) * s.settings['max_encoder']
            )
    ...
    def readMotor(self, index):
        ...
        if self.motorDir[index] > 0:
            val = (
                enc / float(servo.settings["max_encoder"])
                - self.motorMin[index]
            ) / (self.motorMax[index] - self.motorMin[index])
        else:
            val = (
    ...
    def setMotorMin(self):
        ...
        self.motorMin = (encs / float(self.servos[0].settings['max_encoder']
                                     )).tolist()

    def setMotorMax(self):
        amnts, encs = self.readHand()
        self.motorMax = (encs / float(self.servos[0].settings['max_encoder']
                                     )).tolist()
        ...
        if abs(self.servos[i].read_load()) > 80:  #arbitrary load threshold
            ...
            if value < self.servos[i].settings['max_encoder']:
                ...
                if value < 0:
                    ...
                    self.servos[i].move_to_encoder(
                        self.servos[i].settings['max_encoder']
                    )
...
class GR2(OpenHand):
    ...
    def hold(self):
        for i in range(2):
            ...
            self.moveMotor(
                i, amnt
            )  #accounts for possible transition from torque mode
...
class Model_O(OpenHand):
    ...
    motorDir = [
        1, 1, -1, 1
    ]  # one finger is opposite due to placement on the openhand base
    motorMin = [0.0, 0.22, 0.13, 0.27]
    adduct_amount = 0.37
    motorMax = [
        motorMin[0] + adduct_amount, motorMin[1] + max_close,
        motorMin[2] + max_close, motorMin[3] + max_close
    ]  #RX and MX motors may use an offset of 0.48 instead of 0.40
    ...
    def __init__(
        ...
        if (
            mot_offset != self.motorMin
        ):  #update motor mins if initialized to different values
            print('Setting new motor minimums... ')
            self.motorMin = mot_offset
            self.motorMax = [
                self.motorMin[0] + self.adduct_amount,
                self.motorMin[1] + self.max_close,
                self.motorMin[2] + self.max_close,
                self.motorMin[3] + self.max_close
            ]
    ...
    def reset(self):
        ...
        self.moveMotor(
            0, 0.
        )  #moves fingers into lateral pinch mode with fingers orthogonal to thumb
...
#Example why torque control is required for fingertip manipulation
    ...
    def pinch_object_move(
        self,
        delta_amnt=0.03,
        left=True,
        down=False
    ):  #These cannot be the same or else we will move diagonal
        adduct_loc, enc = self.readHand()
        if (adduct_loc[0] > 0.05):
    ...
    def torqueMotor(self, index, val, pos_val=None):
        ...
        if pos_val is None:
            enc = int(
                s.read_encoder() + self.OVERSHOOT * s.settings['max_encoder']
            )
        else:
            ...
            enc = int(
                (
                    pos_val *
                    (self.motorMax[index].modes - self.motorMin[index])
                    + self.motorMin[index]
                ) * s.settings['max_encoder']
            )
...
#shifts object to the edge of the workspace
    ...
    def shift(self, index, val, wait_range=None):
        if index != 1 and index != 2:
            print(
                "[ERR] Can only shift using power grasp with opposing fingers 1 and 2"
            )
            return
        ...
        if val < vals[index]:
            ...
            self.moveMotor(
                other_index, vals[other_index] + (self.motorDir[index]) *
                (vals[index] - val)
            )
        else:
    ...
    def close_jiggle(self, amnt=0.5, da=0.05, nsteps=5, pause=0.25):
        ...
        if s_amnt < 0.5:
            print(
                "[WARNING] Fingers may be spread too far apart for closing motion"
            )
            print(
                "Consider moving fingers to adduction value of greater than 0.5"
            )
        ...
        for i in range(nsteps):
            ...
            amnt_arr = amnt_start + (amnt_goal - amnt_curr
                                    ) * float(i) / nsteps + da_arr * (-1)**i
...
class Model_T42(OpenHand):
    ...
    def __init__(
        ...
        if (
            ...
            self.motorMin = mot_offset
            self.motorMax = [
                self.motorMin[0] + self.max_close,
                self.motorMin[1] + self.max_close
            ]
    ...
    def followTrajectory(self):  #position-based closing mechanism
        act1inputs = np.array(
            [
                0.6777, 0.6854, 0.6820, 0.6762, 0.6705, 0.6531, 0.6363, 0.6256,
                0.6162, 0.6053, 0.6012, 0.5963, 0.5935, 0.5905, 0.5888, 0.5864,
                0.5846, 0.5815, 0.5782, 0.5729, 0.5654, 0.5555, 0.5467, 0.5381,
                0.5317, 0.5283, 0.5274, 0.5272, 0.5271, 0.5232, 0.5179, 0.5077,
                0.4962, 0.4845, 0.4764, 0.4695, 0.4669, 0.4654, 0.4624, 0.4589,
                0.4556, 0.4527, 0.4502, 0.4493, 0.4491, 0.4490, 0.4490, 0.4490,
                0.4490, 0.4490
            ]
        )
        act2inputs = np.array(
            [
                0.6629, 0.6726, 0.6733, 0.6751, 0.6788, 0.6784, 0.6780, 0.6791,
                0.6796, 0.6753, 0.6741, 0.6728, 0.6722, 0.6717, 0.6716, 0.6717,
                0.6722, 0.6726, 0.6738, 0.6751, 0.6756, 0.6754, 0.6758, 0.6758,
                0.6784, 0.6818, 0.6855, 0.6858, 0.6858, 0.6827, 0.6792, 0.6762,
                0.6761, 0.6759, 0.6728, 0.6675, 0.6617, 0.6560, 0.6522, 0.6514,
                0.6524, 0.6533, 0.6535, 0.6520, 0.6504, 0.6497, 0.6492, 0.6491,
                0.6490, 0.6490
            ]
        )
        ...
        for i in range(len(act1inputs)):
            ...
            print(
                "2-link: %.2f" % Decimal(act2inputs[i] * (0.65 - 0.10) + 0.05)
            )
...
class Model_T(OpenHand):
    ...
    def __init__(
        self, port="/dev/ttyUSB0", s1=1, dyn_model="MX", s1_min=motorMin[0]
    ):
    ...
    def close(self, amnt=0.2):  #closing through position mode and torque limit
        ...
        time.sleep(self.pause)
        self.servos[0].move_to_encoder(
            self.servos[0].settings['max_encoder'] - 1
        )
        time.sleep(self.pause)
    ...
    def release(self):  #should work for all previous close cases
        ...
        self.servos[0].apply_torque(
            0.
        )  #allow natural compliance to loosen grasp for tight grip cases
        time.sleep(self.pause * 2)

        self.servos[0].disable_torque_mode()

        self.servos[0].kill_cont_turn()  #back to position mode
        self.servos[0].apply_speed(
            self.servo_speed
        )  #check in case it was in wheel mode
        self.moveMotor(0, self.amnt_release)
